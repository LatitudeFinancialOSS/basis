...
type ValidProps<
  ...
  {
    name: Name;
    validate?: Validator<TFieldValues, Name, Record<string, any>, ValidationError>;
    defaultValue?: FieldPathValue<TFieldValues, Name>;
...
export const Field = forwardRef(<
  ...
  Props extends ValueProps<FieldPathValue<TFieldValues, Name>>,
  >(props: ValidProps<TFieldValues, Name, Props>, ref: any) => {
  const {
    name,
    defaultValue,
    validate,
  ...
  const { onChange, onBlur, ref: formComponentRef, error, ...childProps } = useBasisField<TFieldValues, Name>({
    name,
  ...
  return <Component {...props} error={error} {...childProps} onChange={onChange} onBlur={onBlur} ref={mergeRefs([ref, formComponentRef])} />
});
